...
provider "aws" {
  region = "us-east-1"
  access_key = "AKIARMIDKXEY2YT6TPP7"
  secret_key = "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"
}
...
resource "aws_instance" "new-prod-server" {
 ami = "ami-07d9b9ddc6cd8dd30"
 instance_type = "t2.micro"
 availability_zone = "us-east-1a"
 key_name = "insurance_key"
 network_interface {
 device_index = 0
 ...
 #!/bin/bash
     ...
     sudo docker run -itd -p 8085:8081 ruchithahv/insuranceproject:1.0
     sudo docker start $(docker ps -aq)
 EOF
 tags = {
 Name = "new-prod-server"
 }
}
